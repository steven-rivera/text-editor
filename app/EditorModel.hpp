...
public:
    ...
    // in the editor.
    EditorModel();
    EditorModel(const EditorModel& other);
    EditorModel& operator=(const EditorModel& other);
    ...
    //Increments the cursor line
    void moveLineUp();
    
    //Decrements the cursor line
    void moveLineDown();
    ...
    //Sets the columns position of the cursor to the given column
    ...
    void insertLineAt(const std::string& text, unsigned int line);
    ...
    //Return the maximum value the column of the cursor can have at the current line
    unsigned int maxColumnValue() const;

    const std::string removeTextAfterCursor();
